...
Community-based financial practices like *Chamas* and decentralized investment pools thrive in African and South Asian regions, supporting collective savings and micro-loans while driving economic growth. However, these traditional systems face critical challenges:
...
- Participants earn reputation tokens upon completing a *Chama* round (e.g., staking or restaking cycle).
...
By embracing P2P, we’re scaling a proven model into a global, decentralized powerhouse.
...
- Users stake pooled *Chama* funds into Eigenlayer’s restaking system via the P2P API, receiving LRTs to maintain liquidity.
...
- Plans to enhance API docs with *Chama*-specific examples for devs building similar projects.
...
## Demo Walkthrough
1. **Setup**: Connect a wallet to join a *Chama* pool.
...
*Note*: Due to Testnet(Which is not currently active) constraints, we stop at transaction submission. Post-submission, funds would be staked/restaked, and users would receive LRTs, reputation tokens, and validator privileges, as mocked in the UI.
...
2. Install dependencies:
   ...
   npm install
   ```
3. Configure Eigenlayer API keys (see `.env.example`).
4. Run locally:
   ```bash
   npm start
   ```
